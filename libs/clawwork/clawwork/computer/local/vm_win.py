...
from clawwork.computer.base import (
    BASH_MAX_TIMEOUT_MS,
    SESSION_DIRS,
    SESSION_OUTPUTS_DIR,
    SESSION_TMP_DIR,
...
from clawwork.exceptions import CLIError, VMError, VMMountConflictError

if TYPE_CHECKING:
    from clawwork.computer.local._wsl import WslVM
...
class _VMSessionComputer(AsyncComputerMixin):
    ...
    async def upload(self, src: str, dst: str) -> None:
        ...
        try:
            ...
            # Copy to /tmp first (always writable), then mv into place.
            ...
            await self._vm.shell(
                f"mv {tmp} {shlex.quote(dst)} && "
                f"chown {self._session_name}:{self._session_name} {shlex.quote(dst)} && "
                f"chmod 644 {shlex.quote(dst)}",
                user="root"
            )
        except VMError as e:
            raise CLIError(str(e)) from e
...
class LocalVM:
    ...
    def __init__(self, *, instance: str = "clawwork") -> None:
        ...
        from clawwork.computer.local._wsl import WslVM as _WslVM
            ...
            if not truly_new:
                ...
                for r in resolved_new:
                    ...
                    if probe.exit_code != 0:
                        from clawwork.computer.local._wsl import _session_user_from_guest_mount_path, _win_path_to_wsl
