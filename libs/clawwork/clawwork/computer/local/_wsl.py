...
from clawwork.computer.base import ExecutionMetadata
from clawwork.computer.local._types import ResolvedMount
from clawwork.exceptions import MissingDependencyError, UnsupportedPlatformError, WslError
...
class WslVM:
    ...
    def _config_dir(self) -> Path:
        ...
        data_dir = os.environ.get("CLAWWORK_DATA_DIR")
    ...
    async def _apply_bind_mounts(self) -> None:
        ...
        for m in mounts:
            ...
            skip_chown = os.environ.get("CLAWWORK_WSL_SKIP_SESSION_MOUNT_CHOWN", "").strip().lower() in (
                "1",
                "true",
                "yes",
            )
