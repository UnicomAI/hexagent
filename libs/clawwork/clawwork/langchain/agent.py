"""LangChain agent factory for ClawWork.

Creates an ClawWork agent using LangChain's agent infrastructure.
...
from clawwork.harness import BUILTIN_REMINDERS, DEFAULT_SKILL_PATHS, EnvironmentResolver, PermissionGate, SkillResolver, task_completion_reminder
from clawwork.harness.model import _FALLBACK_COMPACTION_THRESHOLD, ModelProfile
from clawwork.langchain.middleware import AgentMiddleware
from clawwork.langchain.subagent import LangChainSubagentRunner
from clawwork.prompts import FRESH_SESSION, RESUMED_SESSION, compose
from clawwork.tasks import TaskRegistry
from clawwork.tools import SkillTool, TodoWriteTool, create_cli_tools, create_web_tools
from clawwork.tools.task import TaskOutputTool, TaskStopTool
from clawwork.tools.task.agent import AgentTool
from clawwork.trace import init_langchain_tracing
from clawwork.types import AgentContext, CompletionModel
...
if TYPE_CHECKING:
    ...
    from clawwork.harness import Reminder
    from clawwork.harness.definition import AgentDefinition
    from clawwork.mcp import McpClient
    from clawwork.tools.base import BaseAgentTool
    from clawwork.tools.web import FetchProvider, SearchProvider
    from clawwork.types import McpServerConfig, Skill
...
class Agent:
    """An ClawWork agent with managed resources.
...
async def create_agent(
    ...
    *,
    ...
    """Create an ClawWork agent using LangChain.

    ClawWork agents require a LLM that supports tool calling.
    ...
    Args:
        ...
        mcp_servers: MCP server configurations keyed by server name.
            ...
            ClawWork connects to each server, discovers their tools,
    ...
    Returns:
        A configured ClawWork agent. Use as an async context manager
...
async def _connect_mcps(
    ...
    if not mcp_servers:
        return []
    from clawwork.mcp._connector import McpConnector
