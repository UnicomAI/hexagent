...
:class:`~clawwork.types.SubagentRunner` protocol.
...
from clawwork.prompts import SUBAGENT_SESSION, compose
from clawwork.types import AgentContext, SubagentResult
...
if TYPE_CHECKING:
    ...
    from clawwork.harness.permission import PermissionGate
    from clawwork.harness.reminders import Reminder
    from clawwork.harness.skills import SkillResolver
    from clawwork.mcp import McpClient
    from clawwork.tools.base import BaseAgentTool
    from clawwork.types import ApprovalCallback, EnvironmentContext, Skill

logger = logging.getLogger(__name__)

SUBAGENT_EVENT_TAG_PREFIX = "clawwork:subagent:"
"""Prefix for tags added to subagent events.

Each subagent invocation is tagged ``"clawwork:subagent:<task_id>"``.
...
class LangChainSubagentRunner:
    ...
    async def run(
        ...
        *,
        ...
        if has_skill_tool:
            from clawwork.harness.reminders import Reminder as ReminderCls
            from clawwork.harness.reminders import available_skills_reminder
        ...
        # 7. Create graph + run
        graph = _create_langchain_agent(sub_profile.model, middleware=[middleware], name="ClawWork Subagent")
