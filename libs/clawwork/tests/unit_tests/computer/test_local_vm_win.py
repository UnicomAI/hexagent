...
from clawwork.computer.base import SESSION_DIRS, Mount
...
from clawwork.exceptions import VMError, VMMountConflictError
...
def _make_manager(vm: AsyncMock) -> Any:
    ...
    with patch("clawwork.computer.local.vm_win.LocalVM.__init__", return_value=None):
...
class TestMount:
    ...
    async def test_mount_idempotent_self_heals_missing_live_mount(self, tmp_path: Any) -> None:
        ...
        with patch("clawwork.computer.local._wsl._win_path_to_wsl", return_value="/mnt/c/code"):
            await mgr.mount(Mount(source=str(d), target="code"))
