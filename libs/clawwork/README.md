...
The core Python library for building computer-using AI agents. ClawWork is an **agent harness** — the complete runtime layer that gives any LLM **access to a computer via the terminal**, completing tasks the way developers do.
...
```bash
pip install clawwork[langsmith]    # LangSmith tracing
pip install clawwork[braintrust]   # Braintrust observability
pip install clawwork[observe]      # Both
```
...
```python
from clawwork import create_agent
from clawwork.computer import LocalNativeComputer, RemoteE2BComputer
...
| `skill_paths` | `Sequence[str]` | Directories to scan for SKILL.md-based skills. Defaults to `/mnt/skills`, `~/.clawwork/skills`, `.clawwork/skills`. |
...
```python
from clawwork.tools import BaseAgentTool
...
The foundational abstraction. Every agent gets a computer — a pluggable execution environment that abstracts *where* CLI tools run. This is what makes ClawWork agents general-purpose: the same tools work whether the agent is on your laptop, in a VM, or in the cloud.
...
Default discovery paths: `/mnt/skills`, `~/.clawwork/skills`, `.clawwork/skills`.
