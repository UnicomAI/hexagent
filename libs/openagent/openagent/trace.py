...
Example::
    ...
    from openagent.trace import traced
...
def _load_tracers(platforms: list[str]) -> list[Callable[..., Any]]:
    ...
    for name in platforms:
        ...
        if name == "braintrust":
            try:
                mod.init_logger(project="OpenAgent", api_key=os.getenv("BRAINTRUST_API_KEY"))
    ...
    for platform in _active:
        if platform == "langsmith":
            ...
            try:
                ...
                braintrust.init_logger(project="OpenAgent", api_key=os.getenv("BRAINTRUST_API_KEY"))
