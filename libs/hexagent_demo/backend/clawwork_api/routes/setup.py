...
from clawwork_api.paths import data_dir, deps_dir, vm_lima_dir, vm_setup_dir, vm_setup_lite_dir
...
def _wsl_prebuilt_tar_path() -> Path | None:
    ...
    2. Electron extraResources path from ``CLAWWORK_WSL_OFFLINE_DIR`` (if set)
    """
    candidate_dirs: list[Path] = [vm_setup_dir().parent / "wsl" / "prebuilt"]

    offline_dir = os.environ.get("CLAWWORK_WSL_OFFLINE_DIR", "").strip()
    if offline_dir:
...
# Cowork only needs the ``clawwork`` distro to exist; WSL starts it on demand.
...
class _BuildManager(_ProcessManager):
    ...
    async def _start_wsl_instance(
        ...
        *,
        ...
        """Start clawwork distro and optionally retry transient missing-disk errors."""
    ...
    async def _run_lima(self) -> None:
        ...
        yaml_path = vm_lima_dir() / "clawwork.yaml"
    ...
    async def _run_wsl(self) -> None:
        ...
        if _wsl_state_equals(status, _WSL_STOPPED_STATES):
            ...
            if ok:
                ...
                if _looks_like_missing_wsl_disk(err):
                    self._emit("progress", {"step": "creating", "message": "Detected broken WSL distro disk. Recreating ClawWork distro..."})
                    ...
                    u_out_b, u_err_b = await self._communicate_with_heartbeat(
                        proc_unreg,
                        step="creating",
                        message="Removing broken ClawWork WSL distro...",
                    )
        ...
        # Distro does not exist: prefer bundled prebuilt ClawWork rootfs.
        if prebuilt_tar is not None:
            self._emit("progress", {"step": "creating", "message": "Importing bundled ClawWork VM image..."})
            ...
            _, err_b = await self._communicate_with_heartbeat(
                proc_import,
                step="creating",
                message="Importing bundled ClawWork VM image...",
            ...
            self._emit("progress", {"step": "starting", "message": "Starting imported ClawWork WSL distro..."})
            ok, err = await self._start_wsl_instance(
                wsl_exe,
                step="starting",
                message="Starting imported ClawWork WSL distro...",
        ...
        self._emit("progress", {"step": "creating", "message": "Importing ClawWork WSL distro..."})
        proc_import = await asyncio.create_subprocess_exec(
        ...
        _, err_b = await self._communicate_with_heartbeat(
            proc_import,
            step="creating",
            message="Importing ClawWork WSL distro...",
        )
        if proc_import.returncode != 0:
            err = _decode_wsl_output(err_b or b"").strip()
        ...
        self._emit("progress", {"step": "starting", "message": "Starting ClawWork WSL distro..."})
        ok, err = await self._start_wsl_instance(
            wsl_exe,
            step="starting",
            message="Starting ClawWork WSL distro...",
            retries_on_missing_disk=3,
        )
        if ok:
