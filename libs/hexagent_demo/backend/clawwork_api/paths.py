"""Centralised path resolution for the ClawWork demo backend.

All runtime directories are derived from a single root — either
``$CLAWWORK_DATA_DIR`` (when set) or ``~/.clawwork``.  Importing
...
def data_dir() -> Path:
    ...
    Respects ``CLAWWORK_DATA_DIR`` if set, otherwise ``~/.clawwork``.
    """
...
def vm_dir() -> Path:
    """Root directory for VM assets (``libs/clawwork/sandbox/vm/``).
    ...
    import clawwork  # noqa: PLC0415 — deferred to avoid circular import

    return Path(clawwork.__file__).parent.parent / "sandbox" / "vm"
