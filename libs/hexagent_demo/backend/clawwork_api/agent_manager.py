...
Singleton that manages the ClawWork agent lifecycle.
...
class AgentManager:
    """Manages the ClawWork agent lifecycle with per-session caching.
    ...
    async def _verify_session_dir_writable(self, session_name: str, guest_dir: str) -> None:
        ...
        probe = f"{guest_dir.rstrip('/')}/.clawwork_write_probe_{os.getpid()}"
        cmd = (
    ...
    async def _ensure_computer(
        ...
        if mode == "chat":
            ...
            async with self._setup_lock:
                ...
                from clawwork_api.config import load_config
                ...
                computer = RemoteE2BComputer(template="an7tang/clawwork")
                try:
        ...
        if self._vm_manager is None:
            ...
            if sys.platform == "win32":
                from clawwork.computer.local._wsl import _resolve_wsl_exe
        ...
        async with self._setup_lock:
            ...
            try:
                if session_name:
                    ...
                    from clawwork.computer import Mount
    ...
    async def _ensure_vm_manager(self) -> None:
        ...
        async with self._setup_lock:
            ...
            from clawwork.computer.local import LocalVM
    ...
    async def _sync_skills(self) -> None:
        """Sync skill directories (public + private) into the VM via copy.

        This avoids the 'bind mount lost on restart' issue by physically
        copying files into the VM's internal filesystem.
        """
        assert self._vm_manager is not None

        from clawwork_api.paths import bundled_skills_dir, skills_dir
        import shlex
        import sys
        ...
        for subdir, host_path in sync_sources.items():
            if not host_path.is_dir():
                continue

            guest_dir = f"/mnt/skills/{subdir}"
            host_path_str = str(host_path)

            if sys.platform == "win32":
                # Windows: Use wslpath to get the /mnt/ path from DrvFs
                res = await self._vm_manager._vm.shell(f"wslpath {shlex.quote(host_path_str)}")
                if res.exit_code != 0:
                    logger.warning("Failed to resolve WSL path for %s", host_path_str)
                    continue
                wsl_source = res.stdout.strip()
            else:
                # macOS/Linux (Lima): Paths are usually shared 1:1
                wsl_source = host_path_str

            logger.info("Syncing skills: %s -> %s", wsl_source, guest_dir)

            # 1. Create target dir
            # 2. Sync contents (using cp -ru for 'update' behavior if already exists)
            # 3. Ensure root ownership for shared system skills
            sync_cmd = (
                f"mkdir -p {shlex.quote(guest_dir)} && "
                f"cp -ru {shlex.quote(wsl_source)}/. {shlex.quote(guest_dir)}/ && "
                f"chown -R root:root {shlex.quote(guest_dir)}"
            )

            await self._vm_manager._vm.shell(sync_cmd, user="root")
