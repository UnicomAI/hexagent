...
PYINSTALLER_ARGS=(
    ...
    --collect-submodules uniharness_api
    --collect-submodules uniharness
    --collect-data uniharness
    --add-data "skills:skills"
    --add-data "../../uniharness/sandbox/vm:sandbox/vm"
    uniharness_api/server.py
)
...
run_pyinstaller() {
    ...
    cp -r "$BACKEND_DIR/dist/uniharness_api_server" "$ELECTRON_DIR/backend_dist"
}
...
case "$TARGET_ARCH" in
    ...
    universal)
        ...
        lipo -create \
            "$ELECTRON_DIR/backend_dist_arm64/uniharness_api_server" \
            "$ELECTRON_DIR/backend_dist_x64/uniharness_api_server" \
            -output "$ELECTRON_DIR/backend_dist/uniharness_api_server"
