"""Centralised path resolution for the UniHarness demo backend.

All runtime directories are derived from a single root — either
``$UNIHARNESS_DATA_DIR`` (when set) or ``~/.uniharness``.  Importing
...
def data_dir() -> Path:
    ...
    Respects ``UNIHARNESS_DATA_DIR`` if set, otherwise ``~/.uniharness``.
    """
    env = os.environ.get("UNIHARNESS_DATA_DIR")
    if env:
        return Path(env)
...
def vm_dir() -> Path:
    """Root directory for VM assets (``libs/uniharness/sandbox/vm/``).
    ...
    import uniharness  # noqa: PLC0415 — deferred to avoid circular import

    return Path(uniharness.__file__).parent.parent / "sandbox" / "vm"
