...
from uniharness_api.paths import data_dir, deps_dir, vm_lima_dir, vm_setup_dir, vm_setup_lite_dir
...
def _wsl_prebuilt_tar_path() -> Path | None:
    """Return bundled prebuilt WSL rootfs tar if present."""
    prebuilt_dir = vm_setup_dir().parent / "wsl" / "prebuilt"
...
# Cowork only needs the ``uniharness`` distro to exist; WSL starts it on demand.
...
def _wsl_status() -> dict[str, object]:
    ...
    return {
        ...
        "path": wsl,
        "managed": False,
        "reason": last_err or "WSL runtime is not available",
    }
...
async def _install_lima_stream():
    ...
    tmp_dir = tempfile.mkdtemp(prefix="uniharness_lima_")
...
class _BuildManager(_ProcessManager):
    ...
    async def _run_lima(self) -> None:
        ...
        yaml_path = vm_lima_dir() / "uniharness.yaml"
    ...
    async def _run_wsl(self) -> None:
        ...
        if _wsl_state_equals(status, _WSL_STOPPED_STATES):
            ...
            if proc.returncode == 0:
                ...
                if _looks_like_missing_wsl_disk(err):
                    self._emit("progress", {"step": "creating", "message": "Detected broken WSL distro disk. Recreating UniHarness distro..."})
                    ...
                    u_out_b, u_err_b = await self._communicate_with_heartbeat(
                        proc_unreg,
                        step="creating",
                        message="Removing broken UniHarness WSL distro...",
                    )
        ...
        # Distro does not exist: prefer bundled prebuilt UniHarness rootfs.
        if prebuilt_tar is not None:
            self._emit("progress", {"step": "creating", "message": "Importing bundled UniHarness VM image..."})
            ...
            _, err_b = await self._communicate_with_heartbeat(
                proc_import,
                step="creating",
                message="Importing bundled UniHarness VM image...",
            ...
            self._emit("progress", {"step": "starting", "message": "Starting imported UniHarness WSL distro..."})
            ...
            out_b, err_b = await self._communicate_with_heartbeat(
                proc_start,
                step="starting",
                message="Starting imported UniHarness WSL distro...",
            )
        ...
        self._emit("progress", {"step": "creating", "message": "Importing UniHarness WSL distro..."})
        proc_import = await asyncio.create_subprocess_exec(
        ...
        _, err_b = await self._communicate_with_heartbeat(
            proc_import,
            step="creating",
            message="Importing UniHarness WSL distro...",
        )
        if proc_import.returncode != 0:
            err = _decode_wsl_output(err_b or b"").strip()
        ...
        self._emit("progress", {"step": "starting", "message": "Starting UniHarness WSL distro..."})
        proc_start = await asyncio.create_subprocess_exec(
            wsl_exe, "-d", _WSL_INSTANCE, "--", "echo", "ok",
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE,
        ...
        out_b, err_b = await self._communicate_with_heartbeat(
            proc_start,
            step="starting",
            message="Starting UniHarness WSL distro...",
        )
        if proc_start.returncode == 0:
...
_SETUP_MARKER_DIR = "/var/lib/uniharness/setup"
_SETUP_LOG_DIR = "/var/log/uniharness/setup"
_SETUP_VM_DIR = "/tmp/uniharness-setup"
...
class _ProvisionManager(_ProcessManager):
    ...
    async def _run_lima(self, **kwargs: object) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="uniharness_setup_") as tmp:
            ...
            copy_proc = await asyncio.create_subprocess_exec(
                "limactl", "copy", tar_path, f"{_LIMA_INSTANCE}:/tmp/uniharness-setup.tar.gz",
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE,
            )
        ...
        rc, _, err = await _lima_shell(
            ...
            f"sudo tar -xzf /tmp/uniharness-setup.tar.gz -C {_SETUP_VM_DIR} --strip-components=1 && "
            f"rm -f /tmp/uniharness-setup.tar.gz",
            timeout=30,
        )
        if rc != 0:
