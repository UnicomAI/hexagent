...
Singleton that manages the UniHarness agent lifecycle.
...
class AgentManager:
    """Manages the UniHarness agent lifecycle with per-session caching.
    ...
    async def _verify_session_dir_writable(self, session_name: str, guest_dir: str) -> None:
        ...
        probe = f"{guest_dir.rstrip('/')}/.uniharness_write_probe_{os.getpid()}"
        cmd = (
    ...
    async def _ensure_computer(
        ...
        if self._vm_manager is None:
            ...
            if sys.platform == "win32":
                from uniharness.computer.local._wsl import _resolve_wsl_exe
        ...
        async with self._setup_lock:
            ...
            try:
                if session_name:
                    ...
                    from uniharness.computer import Mount
    ...
    async def _get_or_create_agent(
        ...
        async with self._agent_locks[cache_key]:
            ...
            from uniharness.computer.base import SESSION_OUTPUTS_DIR
            from uniharness.harness.definition import AgentDefinition
            from uniharness.harness.model import ModelProfile
            from uniharness.tools import PresentToUserTool
            from uniharness.tools.web import (
                BraveSearchProvider,
                FetchProvider,
    ...
    async def _ensure_vm_manager(self) -> None:
        ...
        async with self._setup_lock:
            ...
            from uniharness.computer.local import LocalVM
    ...
    async def _mount_skills(self) -> None:
        ...
        from uniharness_api.paths import bundled_skills_dir, skills_dir
