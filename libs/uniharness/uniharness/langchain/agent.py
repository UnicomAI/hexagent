"""LangChain agent factory for UniHarness.

Creates an UniHarness agent using LangChain's agent infrastructure.
...
from uniharness.harness import BUILTIN_REMINDERS, DEFAULT_SKILL_PATHS, EnvironmentResolver, PermissionGate, SkillResolver, task_completion_reminder
from uniharness.harness.model import _FALLBACK_COMPACTION_THRESHOLD, ModelProfile
from uniharness.langchain.middleware import AgentMiddleware
from uniharness.langchain.subagent import LangChainSubagentRunner
from uniharness.prompts import FRESH_SESSION, RESUMED_SESSION, compose
from uniharness.tasks import TaskRegistry
from uniharness.tools import SkillTool, TodoWriteTool, create_cli_tools, create_web_tools
from uniharness.tools.task import TaskOutputTool, TaskStopTool
from uniharness.tools.task.agent import AgentTool
from uniharness.trace import init_langchain_tracing
from uniharness.types import AgentContext, CompletionModel
...
if TYPE_CHECKING:
    ...
    from uniharness.harness import Reminder
...
class Agent:
    """An UniHarness agent with managed resources.
...
async def create_agent(
    ...
    *,
    ...
    UniHarness agents require a LLM that supports tool calling.
    ...
    Args:
        ...
        mcp_servers: MCP server configurations keyed by server name.
            ...
            UniHarness connects to each server, discovers their tools,
...
async def _connect_mcps(
    ...
    if not mcp_servers:
        return []
    from uniharness.mcp._connector import McpConnector
