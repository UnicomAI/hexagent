...
:class:`~uniharness.types.SubagentRunner` protocol.
...
from uniharness.prompts import SUBAGENT_SESSION, compose
from uniharness.types import AgentContext, SubagentResult
...
if TYPE_CHECKING:
    ...
    from uniharness.harness.permission import PermissionGate
    from uniharness.harness.reminders import Reminder
    from uniharness.harness.skills import SkillResolver
    from uniharness.mcp import McpClient
    from uniharness.tools.base import BaseAgentTool
    from uniharness.types import ApprovalCallback, EnvironmentContext, Skill

logger = logging.getLogger(__name__)

SUBAGENT_EVENT_TAG_PREFIX = "uniharness:subagent:"
"""Prefix for tags added to subagent events.

Each subagent invocation is tagged ``"uniharness:subagent:<task_id>"``.
...
class LangChainSubagentRunner:
    ...
    async def run(
        ...
        *,
        ...
        if has_skill_tool:
            from uniharness.harness.reminders import Reminder as ReminderCls
            from uniharness.harness.reminders import available_skills_reminder
        ...
        # 7. Create graph + run
        graph = _create_langchain_agent(sub_profile.model, middleware=[middleware], name="UniHarness Subagent")
