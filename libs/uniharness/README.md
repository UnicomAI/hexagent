...
|-----------|------|-------------|
| `model` | `str \| BaseChatModel \| ModelProfile` | LLM specifier string (e.g. `"openai:gpt-5.5"`), a pre-configured LangChain model, or a `ModelProfile` with context window config. |
...
| `skill_paths` | `Sequence[str]` | Directories to scan for SKILL.md-based skills. Defaults to `/mnt/skills`, `~/.uniharness/skills`, `.uniharness/skills`. |
