#!/bin/bash
# =============================================================================
# UniHarness VM Setup (Lite) — Orchestrator
# =============================================================================
...
MARKER_DIR="/var/lib/uniharness/setup"
LOG_DIR="/var/log/uniharness/setup"
LOCK_FILE="/var/run/uniharness-setup.lock"
LOCK_FD=9
...
if grep -qi microsoft /proc/version 2>/dev/null; then
    _UNIHARNESS_DEFAULT_CN_MIRRORS=1
else
    _UNIHARNESS_DEFAULT_CN_MIRRORS=0
fi
UNIHARNESS_USE_CN_MIRRORS="${UNIHARNESS_USE_CN_MIRRORS:-${_UNIHARNESS_DEFAULT_CN_MIRRORS}}"
UNIHARNESS_APT_MIRROR="${UNIHARNESS_APT_MIRROR:-https://mirrors.ustc.edu.cn/ubuntu}"
UNIHARNESS_APT_PORTS_MIRROR="${UNIHARNESS_APT_PORTS_MIRROR:-https://mirrors.ustc.edu.cn/ubuntu-ports}"
UNIHARNESS_PIP_INDEX_URL="${UNIHARNESS_PIP_INDEX_URL:-https://pypi.tuna.tsinghua.edu.cn/simple}"
UNIHARNESS_NPM_REGISTRY="${UNIHARNESS_NPM_REGISTRY:-https://registry.npmmirror.com}"
UNIHARNESS_PLAYWRIGHT_DOWNLOAD_HOST="${UNIHARNESS_PLAYWRIGHT_DOWNLOAD_HOST:-https://npmmirror.com/mirrors/playwright}"
export UNIHARNESS_USE_CN_MIRRORS UNIHARNESS_APT_MIRROR UNIHARNESS_APT_PORTS_MIRROR
export UNIHARNESS_PIP_INDEX_URL UNIHARNESS_NPM_REGISTRY UNIHARNESS_PLAYWRIGHT_DOWNLOAD_HOST
...
pip_install() {
    ...
    local delay=5
    local attempt=1
    local use_cn_mirrors="${UNIHARNESS_USE_CN_MIRRORS:-0}"
    local pip_opts=(
        --timeout 120
        --retries 3
        --no-cache-dir
...
configure_cn_mirrors() {
    if [[ "${UNIHARNESS_USE_CN_MIRRORS}" != "1" ]]; then
        return 0
    fi
    ...
    apt_mirror_esc="${UNIHARNESS_APT_MIRROR//\\/\\\\}"
    apt_mirror_esc="${apt_mirror_esc//&/\\&}"
    apt_mirror_esc="${apt_mirror_esc//|/\\|}"
    apt_ports_mirror_esc="${UNIHARNESS_APT_PORTS_MIRROR//\\/\\\\}"
    apt_ports_mirror_esc="${apt_ports_mirror_esc//&/\\&}"
    apt_ports_mirror_esc="${apt_ports_mirror_esc//|/\\|}"

    if [[ -f /etc/apt/sources.list ]]; then
        cp -n /etc/apt/sources.list /etc/apt/sources.list.uniharness.bak 2>/dev/null || true
        sed -Ei \
    ...
    if [[ -f /etc/apt/sources.list.d/ubuntu.sources ]]; then
        cp -n /etc/apt/sources.list.d/ubuntu.sources /etc/apt/sources.list.d/ubuntu.sources.uniharness.bak 2>/dev/null || true
        sed -Ei \
...
[global]
index-url = ${UNIHARNESS_PIP_INDEX_URL}
...
EOF
    ...
    cat >/etc/profile.d/uniharness-mirrors.sh <<EOF
export UNIHARNESS_USE_CN_MIRRORS=1
export PIP_INDEX_URL='${UNIHARNESS_PIP_INDEX_URL}'
export NPM_CONFIG_REGISTRY='${UNIHARNESS_NPM_REGISTRY}'
export PLAYWRIGHT_DOWNLOAD_HOST='${UNIHARNESS_PLAYWRIGHT_DOWNLOAD_HOST}'
EOF
}
