"""End-to-end unit tests for uniharness with fake LLM models."""
...
from uniharness.langchain import create_agent
...
class TestDeepAgentEndToEnd:
    ...
    async def test_deep_agent_with_fake_llm_basic(self) -> None:
    ...
    async def test_deep_agent_with_fake_llm_bash_tool(self) -> None:
        ...
        This test verifies that a deepagent can use the built-in bash
        tool with a fake LLM model.
        """
        # Create a fake model that uses bash tool
        model = FixedGenericFakeChatModel(
            messages=iter(
                [
                    AIMessage(
    ...
    async def test_deep_agent_with_string_model_name(self) -> None:
        ...
        with patch("uniharness.langchain.agent.init_chat_model", return_value=fake_model):
