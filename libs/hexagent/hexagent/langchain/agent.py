"""LangChain agent factory for HexAgent.

Creates an HexAgent agent using LangChain's agent infrastructure.
...
from hexagent.harness import BUILTIN_REMINDERS, DEFAULT_SKILL_PATHS, EnvironmentResolver, PermissionGate, SkillResolver, task_completion_reminder
from hexagent.harness.model import _FALLBACK_COMPACTION_THRESHOLD, ModelProfile
from hexagent.langchain.middleware import AgentMiddleware
from hexagent.langchain.subagent import LangChainSubagentRunner
from hexagent.prompts import FRESH_SESSION, RESUMED_SESSION, compose
from hexagent.tasks import TaskRegistry
from hexagent.tools import SkillTool, TodoWriteTool, create_cli_tools, create_web_tools
from hexagent.tools.task import TaskOutputTool, TaskStopTool
from hexagent.tools.task.agent import AgentTool
from hexagent.trace import init_langchain_tracing
from hexagent.types import AgentContext, CompletionModel
...
if TYPE_CHECKING:
    ...
    from hexagent.harness import Reminder
    ...
    from hexagent.tools.web import FetchProvider, SearchProvider
    from hexagent.types import McpServerConfig, Skill
...
class Agent:
    """An HexAgent agent with managed resources.
...
async def create_agent(
    ...
    *,
    ...
    """Create an HexAgent agent using LangChain.

    HexAgent agents require a LLM that supports tool calling.
    ...
    Args:
        ...
        mcp_servers: MCP server configurations keyed by server name.
            ...
            HexAgent connects to each server, discovers their tools,
    ...
    Returns:
        A configured HexAgent agent. Use as an async context manager
